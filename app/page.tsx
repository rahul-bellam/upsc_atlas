...
export default function Page(){
  ...
  async function searchPlace(){
    ...
    try{
      ...
      if(res.data && res.data.lat && res.data.lng){
        setPosition({lat:res.data.lat,lng:res.data.lng}); setZoom(10)
  ...
  async function enrich(){
    ...
    try{
      const res = await axios.get(`/api/ai/enrich?q=${encodeURIComponent(marker.display_name)}&provider=${aiProvider}`)
      alert('AI summary (top):\n' + (res.data.summary || JSON.stringify(res.data)))
    }catch(e){ console.error(e); alert('AI call failed') }
  }
  ...
  return (
    <div className="w-screen h-screen flex">
      <aside className="w-96 p-4 bg-white shadow h-full overflow-auto">
        ...
        <div className="mt-3">
          <input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search place" className="w-full border p-2 rounded" />
          <div className="flex gap-2 mt-2">
            <button onClick={searchPlace} className="px-3 py-1 bg-black text-white rounded">Search (Geocode)</button>
            <button onClick={enrich} className="px-3 py-1 bg-blue-600 text-white rounded">AI Enrich</button>
          </div>
        </div>

        <div className="mt-4">
          <label className="block text-sm">AI Provider</label>
          <select value={aiProvider} onChange={(e)=>setAiProvider(e.target.value)} className="mt-1 border p-1 rounded w-full">
            <option value="openai">OpenAI</option>
            <option value="deepseek">DeepSeek</option>
          </select>
        </div>

        <div className="mt-6 text-xs text-slate-600">Use OpenAI for geocoding; choose provider for enrichment.</div>
      </aside>
